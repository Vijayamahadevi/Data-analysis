...
def draw_bar_plot():
    df_bar = df.copy()
    df_bar["year"] = df.index.year.values # type: ignore
    df_bar["month"] = df.index.month_name() # type: ignore

    fig, ax = plt.subplots(figsize=(15,5))
    
    ax = sns.barplot(x="year", hue="month", y="value", data=df_bar, hue_order = months, ci=None ) # type: ignore
...
def draw_box_plot():
    ...
    df_box['monthnumber'] = df.index.month # type: ignore
